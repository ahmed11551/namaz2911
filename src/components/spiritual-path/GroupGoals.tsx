    ...
    <div className="space-y-6">
      ...
      {groups.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {groups.map((group) => {
            const goal = goals.find((g) => g.id === group.goal_id);
            const progressPercent = goal && goal.target_value > 0
                      ...
                      <div>
                        <div className="flex items-center justify-between text-sm mb-1">
                          ...
                          <span className="font-semibold">
                            {groupGoal.target_value > 0 ? Math.min(100, Math.round(((groupGoal.current_value || 0) / groupGoal.target_value) * 100)) : 0}%
                          </span>
                        </div>
                        <Progress
                          value={groupGoal.target_value > 0 ? Math.min(100, Math.round(((groupGoal.current_value || 0) / groupGoal.target_value) * 100)) : 0}
                          className="h-3"
                        />
                      </div>
