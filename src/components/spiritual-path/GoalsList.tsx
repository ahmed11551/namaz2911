...
export const GoalsList = () => {
  ...
  const getDailyPlanStatus = (goal: Goal): { status: "on_track" | "behind" | "ahead"; message: string } => {
    ...
    const expectedProgress = (goal.target_value || 0) - (daysRemaining * (goal.daily_plan || 0));
    ...
    if (actualProgress >= expectedProgress) {
      ...
      const needed = Math.ceil(((goal.target_value || 0) - (goal.current_value || 0)) / Math.max(1, daysRemaining));
      return { 
        status: "behind", 
