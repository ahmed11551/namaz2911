  ...
  const analyzeUserBehavior = async (): Promise<SmartTemplate[]> => {
    ...
    try {
      ...
      if (quranGoals.length === 0) {
        ...
        if (activeQuranGoal && activeQuranGoal.target_value > 0) {
          const progress = ((activeQuranGoal.current_value || 0) / activeQuranGoal.target_value) * 100;
          if (progress < 30) {
            suggestions.push({
              id: "quran_boost",
      ...
      if (recentQuranGoals.length > 0 && quranGoals.length === 0) {
        ...
        const avgPagesPerDay = recentQuranGoals.reduce((sum, goal) => {
          const days = goal.end_date && goal.start_date
            ...
            : 30;
          return sum + ((goal.target_value || 0) / Math.max(1, days));
